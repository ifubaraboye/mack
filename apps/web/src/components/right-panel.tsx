...
import { FileTypeIcon, MackIcon, type MackIconName } from '@/components/mack-icon'
    ...
    >
      ...
      <header className="flex h-12 shrink-0 items-center gap-1.5 px-2.5 pr-3.5">
        ...
        {tabs.length > 0 && (
          ...
          >
            <MackIcon className="size-3.5" name="plus" />
          </ControlMenu>
        )}
  ...
  return (
    ...
    >
      {(tab.surface === 'files' || tab.surface === 'file') && tab.selectedFile
        ...
        : <MackIcon className="size-[13px] text-[var(--text-secondary)]" name={icon} />}
      ...
      >
        <MackIcon className="size-2.5 text-[var(--text-tertiary)]" name="x" />
      </button>
    </div>
  )
}
    ...
    <div className="flex min-h-0 flex-1 items-center justify-center px-5 pb-8">
      <div className="w-full max-w-[420px] text-center">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 text-left">
          <PanelCard icon={<MackIcon className="size-[18px]" name="terminal" />} label={t('right_panel.terminal')} description={t('right_panel.terminal_description')} onClick={() => onSelect('terminal')} />
          <PanelCard icon={<MackIcon className="size-[18px]" name="folder" />} label={t('right_panel.files')} description={t('right_panel.files_description')} onClick={() => onSelect('files')} />
          <PanelCard icon={<MackIcon className="size-[18px]" name="fileDiff" />} label={t('right_panel.diff')} description={t('right_panel.diff_description')} onClick={() => onSelect('changes')} />
        </div>
      </div>
    </div>
  )
...
function TreeRow({
  ...
  return (
    ...
    >
      {entry.isDir
        ? expanded
          ? <MackIcon className="size-2.5 shrink-0 text-[var(--text-ghost)]" name="chevronDown" />
          : <MackIcon className="size-2.5 shrink-0 text-[var(--text-ghost)]" name="chevronRight" />
        : <span className="size-2.5 shrink-0" />}
      {entry.isDir
        ? <MackIcon className="size-3.5 shrink-0 text-[var(--text-tertiary)]" name="folder" />
      ...
      <div className="flex min-h-0 flex-1">
        ...
        >
          ...
          <label className="flex h-11 shrink-0 items-center gap-2 border-b px-2">
            <MackIcon className="size-[13px] shrink-0 text-[var(--text-tertiary)]" name="search" />
            <input
          ...
          <Virtuoso
            ...
            itemContent={(index, row) => row.kind === 'directory' ? (
              ...
              >
                <MackIcon className="size-2.5 shrink-0 text-[var(--text-ghost)]" name={row.expanded ? 'chevronDown' : 'chevronRight'} />
                <MackIcon className="size-[13px] shrink-0 text-[var(--text-tertiary)]" name="folder" />
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex min-h-10 items-center gap-2 border-b px-3 text-[11px]">
        ...
        <button aria-label={t('diff.refresh')} className="rounded p-1 hover:bg-accent" type="button" onClick={() => void diff.refetch()}>
          <MackIcon className={cn('size-3.5', diff.isFetching && 'motion-safe:animate-spin')} name="rotateCw" />
        </button>
      </div>
      {reviewContent}
    </div>
  ...
  if (!session || !item) {
    return (
      <div className="grid min-h-0 flex-1 place-items-center p-6 text-center">
        <div>
          <MackIcon className="mx-auto size-[22px] text-[var(--text-ghost)]" name={backgroundWorkKindIcon(workKey.kind)} />
    ...
    <div className="min-h-0 flex-1 overflow-auto p-3">
      <div className="overflow-hidden rounded-[9px] border bg-card">
        <div className="flex min-h-[54px] items-center gap-2.5 px-[11px] py-2">
          <MackIcon className="size-[15px] text-[var(--text-secondary)]" name={backgroundWorkKindIcon(item.key.kind)} />
          ...
          {stoppable && (
            ...
            >
              <MackIcon className="size-[11px] text-destructive" name="stopFilled" />
              {t('background.stop')}
            </Button>
          )}
        </div>
...
function BackgroundStatusIcon({ status }: { status: BackgroundWorkStatus }) {
  const icon: MackIconName = status === 'completed'
    ? 'check'
    : status === 'failed'
      ? 'x'
      : status === 'lost'
...
function backgroundWorkKindIcon(kind?: BackgroundWorkKey['kind']): MackIconName {
