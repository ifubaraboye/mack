  ...
  return (
    <TranscriptLinkContext.Provider value={onOpenLink ?? (() => false)}>
      ...
      {!atBottom && !empty && (
        ...
        >
          <MackIcon className="size-4" name="arrowDown" />
        </button>
      )}
      </div>
...
function TurnFold({
  ...
  return (
    <div className="flex h-6 w-full items-center gap-2.5">
      ...
      >
        {label}
        <MackIcon className="size-2.5" name={expanded ? 'chevronDown' : 'chevronRight'} />
      </button>
      <div className="h-px flex-1 bg-border" />
    </div>
  )
    ...
    >
      ...
      <div className="mt-[7px] flex justify-end gap-1.5">
        ...
        <button
          ...
          type="submit"
        >
          {pending && <MackIcon className="size-3 motion-safe:animate-spin" name="loaderCircle" />}
          {t('common.send')}
        </button>
      </div>
    </form>
...
function MessageFooter({
  ...
  return (
    <div className={cn(
      ...
      >
        <MackIcon className="size-3.5" name={copied ? 'check' : 'copy'} />
      </button>
        ...
        <ContextMenu.Positioner className="z-[100] outline-none">
          ...
          >
            {selectedText && (
              ...
              >
                <MackIcon className="size-3" name="copy" /> {t('common.copy_selection')}
              </ContextMenu.Item>
            )}
            <ContextMenu.Item
              className="waku-menu-item"
            ...
            >
              <MackIcon className="size-3" name="copy" /> {t('common.copy_message_title')}
            </ContextMenu.Item>
            {copyToComposer && (
              <ContextMenu.Item
                className="waku-menu-item"
              ...
              >
                <MackIcon className="size-3" name="compose" /> {t('common.copy_to_composer')}
              </ContextMenu.Item>
            )}
            {code && (
              <ContextMenu.Item
              ...
              >
                <MackIcon className="size-3" name="copy" /> {t('common.copy_code')}
              </ContextMenu.Item>
            )}
            {rewindAction && (
              <>
  ...
  return (
    <div className="min-w-0 text-[12px] text-[var(--text-tertiary)]">
      ...
      >
        ...
        <MackIcon className="size-2.5 shrink-0" name={expanded ? 'chevronDown' : 'chevronRight'} />
      </button>
      {expanded && (
    ...
    <div className="min-w-0 overflow-hidden rounded-[9px] border bg-[var(--activity-surface)]">
      <div className="flex h-7 w-full min-w-0 items-center">
        <button
          ...
          onClick={() => setExpanded((value) => !value)}
        >
          <MackIcon className="size-3 shrink-0 text-[var(--text-tertiary)]" name={iconName} />
      ...
      {section.label && (
        <div className="flex h-5 items-center justify-between font-medium">
          ...
          {section.content && (
            ...
            >
              <MackIcon className="size-[11px]" name={copied ? 'check' : 'copy'} />
            </button>
          )}
        </div>
      )}
...
function ActivityState({
  ...
  if (hasDetail) return <MackIcon className="size-2.5 text-[var(--text-tertiary)]" name={expanded ? 'chevronDown' : 'chevronRight'} />
  if (activity.reasoning) return null
  if (activity.failed) return <MackIcon label={t('background.status.failed')} className="size-3 text-destructive" name="alert" />
    ...
    <div className="overflow-hidden rounded-xl border bg-accent">
      <div className="flex min-h-[58px] items-center gap-2.5 px-3 py-[9px]">
        <span className="grid size-9 shrink-0 place-items-center rounded-[9px] bg-[var(--raised)]">
          <MackIcon className="size-4 text-[var(--text-tertiary)]" name="fileDiff" />
        </span>
        ...
        {onReview && (
          <button
            ...
            onClick={onReview}
          >
            <MackIcon className="size-3 text-[var(--text-tertiary)]" name="fileDiff" />
      ...
      <div className="border-t">
        ...
        {files.length > 3 && (
          ...
          >
            ...
            <MackIcon className="size-[11px] text-[var(--text-tertiary)]" name={expanded ? 'chevronDown' : 'chevronRight'} />
          </button>
        )}
      </div>
    </div>
...
function Attachment({ attachment }: { attachment: MessageAttachment }) {
  ...
  return (
    ...
    >
      {attachment.is_dir
        ? <MackIcon className="size-[18px] text-[var(--text-tertiary)]" name="folder" />
...
function activityIcon(activity: ActivityItem): MackIconName {
