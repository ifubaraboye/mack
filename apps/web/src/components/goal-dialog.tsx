...
export function GoalDialog({
  ...
  const statusAction = goal && !replace
    ? goal.status === 'active'
      ? { status: 'paused' as const, label: t('goal.pause'), icon: 'stop' as MackIconName }
      : goal.status === 'paused' || goal.status === 'blocked' || goal.status === 'usageLimited'
        ? { status: 'active' as const, label: t('goal.resume'), icon: 'arrowUp' as MackIconName }
        : null
    : null
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogTitle className="flex h-12 items-center gap-2.5 px-4 text-sm font-normal">
          <MackIcon className="size-[15px]" name="target" />
