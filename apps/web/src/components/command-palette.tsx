...
import { ProviderIcon, PROVIDERS, providerMeta, MackIcon, type MackIconName } from '@/components/mack-icon'
        ...
        <div className="flex h-[60px] shrink-0 items-center border-b px-[19px]">
          ...
          {view === 'resume' && (resultsPending ? (
            ...
            >
              ...
              <MackIcon className="size-3 text-[var(--text-tertiary)]" name="chevronDown" />
            </button>
          ))}
        </div>
      ...
      <div className={cn(header && item.section === 'providers' && 'pt-2')} key={item.id}>
        ...
        >
          <span className="grid size-5 shrink-0 place-items-center text-[var(--text-secondary)]">
            {item.pending
              ? <MackIcon className="size-4 animate-spin motion-reduce:animate-none" name="loaderCircle" />
              : item.provider
              ? <ProviderIcon className="size-4" provider={item.provider} />
              : item.icon && <MackIcon className="size-4" name={item.icon} />}
          </span>
