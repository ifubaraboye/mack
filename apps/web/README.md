...
Browser client for an existing Mack daemon. The Cloudflare Worker serves the
...
```sh
MACK_DAEMON_TOKEN=replace-me cargo run -p waku-daemon --bin mack-daemon -- \
  --bind 127.0.0.1:34123 \
  --allow-origin http://localhost:3001
```

Mack Desktop can expose the daemon it manages from Settings → Daemon, where you
