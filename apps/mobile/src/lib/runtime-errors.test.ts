...
describe('isDaemonDisconnectError', () => {
  test('recognizes the client transport messages emitted during a dropped link', () => {
    expect(isDaemonDisconnectError(new Error('Mack daemon disconnected'))).toBe(true);
    expect(isDaemonDisconnectError('Mack daemon is disconnected')).toBe(true);
    expect(isDaemonDisconnectError('Mack client disconnected')).toBe(true);
  });

  test('does not swallow a real daemon or agent error', () => {
    expect(isDaemonDisconnectError('timed out waiting for Mack daemon')).toBe(false);
...
describe('clearDaemonDisconnectErrors', () => {
  test('removes only stale transport errors after reconnecting', () => {
    const errors = {
      disconnected: 'Mack daemon disconnected',
