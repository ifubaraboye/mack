# Mack development guidance

## Development runtime

- Assume `bun ./scripts/dev.ts` is already running and owns the current
  `Mack Debug.app` process. Source changes are rebuilt, signed, and relaunched
...
- During normal development and UI validation, do not run
  ...
  `Mack Debug.app`. Quitting the app also stops the watcher.
...
- Treat performance as a product requirement, not a follow-up. Mack is a native
...
- Use [Zed](https://github.com/zed-industries/zed) source code as a reference
  ...
  APIs match what Mack builds against.
...
- Use the reference as behavioral and design evidence, not as an instruction to
  reproduce web-specific interaction patterns or known bugs. Mack should keep
