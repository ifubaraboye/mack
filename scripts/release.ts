...
const appName = "Mack";
const executableName = "Mack";
const jsReplExecutableName = "mack_js_repl";
const daemonExecutableName = "mack-daemon";
const computerUseHelperName = "Mack Computer Use";
const packageName = "mack";
const defaultNotaryProfile = "NOTARY";
const projectRoot = resolve(import.meta.dir, "..");

const help = `Build, notarize, and publish a production release of Mack.
...
https://releases.mack.sh. One-time setup lives in RELEASING.md.

Options:
  --local                       Build, notarize, and write the DMG + zip
                                without publishing to R2
  --force                       Publish even if this version is already in R2
  --output <path>               DMG output path (default: dist/Mack-<version>.dmg)
  --signing-identity <name>     Developer ID Application identity selector
                                (or MACK_SIGNING_IDENTITY; required unless --adhoc)
  --notary-profile <name>       notarytool keychain profile
                                (default: NOTARY; or MACK_NOTARY_PROFILE)
  --build-number <number>       CFBundleVersion override (or MACK_BUILD_NUMBER;
                                default derives a monotonic number from the
                                Cargo version)
  --volume-name <name>          Mounted DMG name (default: Mack)
  --skip-build                  Reuse target/release/mack, mack_js_repl, and
                                mack-daemon
...
Environment:
  MACK_SIGNING_IDENTITY         Developer ID Application identity selector
  MACK_ANALYTICS_ENDPOINT       analytics endpoint embedded at build time
  MACK_ANALYTICS_WEBSITE_ID     analytics website ID embedded at build time
  MACK_R2_REMOTE                rclone remote name (default: r2)
  MACK_R2_BUCKET                R2 bucket name (default: mack-releases)
  MACK_DOWNLOAD_URL_PREFIX      base URL served by the bucket
                                (default: ${defaultDownloadUrlPrefix})
  MACK_HISTORY_COUNT            prior archives pulled for deltas (default: 15)
  MACK_NO_HISTORY=1             skip pulling prior archives (no deltas)
  SPARKLE_BIN                   Sparkle tools dir (default: the bundle.sh cache
                                under .mack-cache/sparkle)
...
const configuredSigningIdentity =
  values["signing-identity"] ?? process.env.MACK_SIGNING_IDENTITY;
const notaryProfile =
  values["notary-profile"] ??
  process.env.MACK_NOTARY_PROFILE ??
  defaultNotaryProfile;
const explicitBuildNumber =
  values["build-number"] ?? process.env.MACK_BUILD_NUMBER;
const analyticsEndpoint = process.env.MACK_ANALYTICS_ENDPOINT?.trim();
const analyticsWebsiteId = process.env.MACK_ANALYTICS_WEBSITE_ID?.trim();
...
const r2Remote = process.env.MACK_R2_REMOTE ?? "r2";
const r2Bucket = process.env.MACK_R2_BUCKET ?? "mack-releases";
...
const downloadUrlPrefix =
  process.env.MACK_DOWNLOAD_URL_PREFIX ?? defaultDownloadUrlPrefix;
const historyCount = Number(process.env.MACK_HISTORY_COUNT ?? "15");
const skipHistory = process.env.MACK_NO_HISTORY === "1";
...
if (!adhoc && !configuredSigningIdentity) {
  throw new Error(
    "Set MACK_SIGNING_IDENTITY or pass --signing-identity (or use --adhoc).",
  );
}
...
if (!Number.isSafeInteger(historyCount) || historyCount < 0) {
  throw new Error("MACK_HISTORY_COUNT must be a non-negative integer.");
}
if (!values["skip-build"] && (!analyticsEndpoint || !analyticsWebsiteId)) {
  throw new Error(
    "Set MACK_ANALYTICS_ENDPOINT and MACK_ANALYTICS_WEBSITE_ID before building a release.",
  );
}
    ...
    if (detail.includes("directory not found")) {
      throw new Error(
        `R2 bucket "${r2Bucket}" does not exist on remote "${r2Remote}". ` +
          ...
          "releases.mack.sh custom domain (see RELEASING.md), then re-run.",
      );
    }
  ...
  const requests = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "mack-release", version: "1" },
      },
    },
...
try {
  ...
  await $`env MACK_CODESIGN_IDENTITY=${identity} MACK_ANALYTICS_ENDPOINT=${analyticsEndpoint ?? ""} MACK_ANALYTICS_WEBSITE_ID=${analyticsWebsiteId ?? ""} MACK_SKIP_CARGO_BUILD=${values["skip-build"] ? "1" : "0"} ${join(projectRoot, "scripts", "bundle.sh")} release`;
  ...
  temporaryDirectory = await mkdtemp(join(tmpdir(), "mack-dmg-"));
  ...
  // archive as Mack-<version>.md; generate_appcast links it as the update's
  ...
  if (publishing) {
    ...
    console.log(`\nMack ${version} (build ${buildNumber}) is live:`);
