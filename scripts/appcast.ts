...
//
// <updates-dir> holds the packaged archives (e.g. Mack-0.2.0.zip) plus any
...
//   MACK_DOWNLOAD_URL_PREFIX   base URL for enclosure links
import { existsSync, readdirSync } from "node:fs";
import { join, resolve } from "node:path";

const projectRoot = resolve(import.meta.dir, "..");

export const defaultDownloadUrlPrefix = "https://releases.mack.sh/";

/** Locate Sparkle's `generate_appcast`: SPARKLE_BIN first, then the pinned
 *  distribution scripts/bundle.sh caches under .mack-cache, then PATH. */
export function findGenerateAppcast(): string | null {
  ...
  const cacheRoot = join(projectRoot, ".mack-cache", "sparkle");
  ...
  if (!generator) {
    throw new Error(
      "generate_appcast not found. Run scripts/bundle.sh once to populate " +
        ".mack-cache/sparkle, or set SPARKLE_BIN to a Sparkle tools bin/ dir.",
    );
  }
  console.log(`Using: ${generator}`);
  // Same prefix for both: archives and the Mack-<version>.md release notes are
