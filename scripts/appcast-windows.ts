...
//
// <assets-dir> holds this release's `Mack-<version>-<arch>-Setup.exe` files.
...
export async function generateWindowsAppcasts(
  ...
  for (const arch of architectures) {
    const installer = `Mack-${version}-${arch}-Setup.exe`;
  ...
  if (written.length === 0) {
    throw new Error(`No Mack-${version}-<arch>-Setup.exe found in ${assetsDir}`);
  }
  return written;
}
