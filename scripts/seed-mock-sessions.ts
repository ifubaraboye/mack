...
const CODE_SAMPLES: { language: string; lines: string[] }[] = [
  ...
  },
  {
    ...
    lines: [
      ...
      "hyperfine --warmup 3 'target/debug/mack --bench transcript'",
...
function buildLog(random: Random, lines: number): string {
  ...
  for (let index = 0; index < lines; index += 1) {
    ...
    if (roll < 0.08) {
      ...
      out.push(
        `   Compiling ${pick(random, ["gpui", "mack", "rusqlite", "pulldown-cmark", "smol"])} v${int(random, 0, 3)}.${int(random, 0, 40)}.${int(random, 0, 9)}`,
      );
    } else {
      out.push(
...
function blobReferences(databasePath: string): string[] {
  ...
  for (const shard of readdirSync(blobsRoot)) {
    ...
    for (const file of files) {
      if (/\.(png|jpe?g|gif|webp)$/i.test(file)) references.push(`mack-blob:${file}`);
    }
  }
  return references;
}
    ...
    turns.push({
      ...
      checkpoint: chance(random, 0.7)
        ? {
            ...
            git_ref: `refs/mack/session-${sessionId}-turn-${turnIndex + 1}`,
            status: "ready",
