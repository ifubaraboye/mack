...
#   WAKU_APPIMAGE_PROFILE   cargo profile to bundle (default: release)
#   WAKU_LINUXDEPLOY_URL    linuxdeploy AppImage URL (arch default below)
#   WAKU_APPIMAGETOOL_URL   appimagetool AppImage URL (arch default below)
#
# Output:
#   target/<profile>/Waku-<version>-<arch>.AppImage (+ .sha256 sidecar)
#
# Notes:
# - The AppImage intentionally omits share/waku/self-update-v1, so the in-app
...
profile="${WAKU_APPIMAGE_PROFILE:-release}"
...
out="$target_dir/$profile/Waku-${version}-${arch}.AppImage"
...
mkdir -p "$prefix"
bun scripts/cua-driver.ts bundle "$prefix/bin" "$prefix/share/waku" "$profile"
install -Dm755 "$target_dir/$profile/waku" "$prefix/bin/waku"
install -Dm755 "$target_dir/$profile/waku-updater" "$prefix/bin/waku-updater"
install -Dm755 "$target_dir/$profile/waku-daemon" "$prefix/bin/waku-daemon"
install -Dm644 resources/linux/sh.waku.desktop \
  "$prefix/share/applications/sh.waku.desktop"
install -Dm644 website/public/app-icon.png \
  "$prefix/share/icons/hicolor/256x256/apps/sh.waku.png"
install -Dm644 LICENSE "$prefix/share/licenses/waku/LICENSE"
...
linuxdeploy_url="${WAKU_LINUXDEPLOY_URL:-$linuxdeploy_default}"
appimagetool_url="${WAKU_APPIMAGETOOL_URL:-$appimagetool_default}"
...
"$linuxdeploy_bin" --appimage-extract-and-run \
  --appdir "$appdir" \
  --executable "$appdir/usr/bin/waku" \
  --desktop-file "$root/resources/linux/sh.waku.desktop" \
  --icon-file "$root/website/public/app-icon.png"

# appimagetool requires the top-level desktop file + icon to match Icon=.
test -f "$appdir/sh.waku.desktop"
test -f "$appdir/sh.waku.png" || test -f "$appdir/.DirIcon"
if [ ! -e "$appdir/.DirIcon" ]; then
  ln -sf sh.waku.png "$appdir/.DirIcon"
fi
