...
// Sparkle-format feeds. Mack's native Linux updater reads this same compact
...
export async function generateLinuxAppcasts(
  ...
  for (const arch of architectures) {
    const archive = `mack-${version}-${targetTriple(arch)}.tar.gz`;
  ...
  if (written.length === 0) {
    throw new Error(`No mack-${version}-<target>.tar.gz found in ${assetsDir}`);
  }
  return written;
}
