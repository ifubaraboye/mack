...
Mack is a fast, native desktop app for working with local coding agents. It is
...
On macOS, [download the signed `.dmg`](https://mack.sh). It updates itself.
...
On Windows, run `Mack-<version>-<arch>-Setup.exe` from the
[latest release](https://github.com/ifubaraboye/mack/releases/latest). It installs
...
Install and authenticate at least one supported agent CLI before starting Mack.
Mack detects available CLIs automatically and uses each provider's native
...
- Store app state locally, with no Mack account or remote service required.

## Architecture

The native desktop is an RPC client of the standalone `mack-daemon` process.
Provider sessions run in [`waku-core`](crates/waku-core), behind the
authenticated, versioned WebSocket contract in
[`waku-protocol`](crates/waku-protocol). Mack Desktop depends on
...
`~/.mack/projects/<date>/<slug>`. The daemon moves workspaces created by the
older `~/.mack/<date>/<slug>` layout on first load.

Configuration ownership is separate too: the Release desktop writes
`~/.mack/app.json`, while Debug stays isolated at `temp/app.json`. Daemon
provider and Computer Use settings live in `~/.mack/settings.json`. The
...
When connected to a daemon managed outside the desktop process, Mack never
...
Release apps bundle and sign `mack-daemon`. Development keeps the daemon at
`target/debug/mack-debug-daemon`, allowing provider-only edits to rebuild and
replace the daemon without relaunching Mack Debug.
...
Mack is licensed under the [GNU General Public License v3.0 only](LICENSE).
