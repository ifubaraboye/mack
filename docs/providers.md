...
How Mack talks to each coding agent: the process it launches, the wire protocol
...
polled off disk, and the one Mack generates itself — is in
[titles.md](titles.md).
...
`Mack::runtimes` keyed by session id. Runtimes are per session, not per view:
...
([opencode_session.rs](../crates/waku-core/src/opencode_session.rs)). Mack quitting without
...
| Interactive approvals | yes | no | no (has them; Mack runs `--yolo`) | yes | no | yes | yes | yes | yes | yes |
...
Codex can send Mack requests (approvals) and Mack answers them by id. Three
...
2. `initialized`.
3. `skills/extraRoots/set` when Computer Use is on, so Mack's bundled skill is
...
per-peer, the reader only treats method-less messages as replies to Mack's own
...
**Computer Use** — `-c mcp_servers.mack_js_repl.command=…` registers Mack's
QuickJS MCP server, with several `-c` flags disabling Codex's own external
computer-use plugin/MCP/skill so only Mack's `js` / `js_reset` surface is
visible.
...
| Whole-session copy | in place | only at launch, so Mack shells out (see below) |
| Computer Use | Mack's Pi extension | none — Oh My Pi ships its own `/computer` |
...
Mack stamps each request with a string id (`mack-<n>`) and Pi answers with
...
| `extension_ui_request` | auto-cancelled — Mack has no UI for extension prompts |
...
*does* have one, which Mack's `--yolo` then bypasses — the restriction is Mack's
...
performs in place. Oh My Pi only copies at launch, so Mack shells out to a
...
**Computer Use** — Pi only: `--extension <mack pi extension>` and
`--skill <SKILL.md>`, with the REPL and helper paths passed through the
environment. Mack's bridge is written against Pi's extension API, and Oh My Pi
...
| `user` with `isReplay: true` | ignored — Mack's own prompt echoed by `--replay-user-messages` |
| `result` | `TurnFinished` |
...
`permission_suggestions`. Mack answers with a `control_response` whose result is
...
**Native checkpoints** — after each turn Mack reads Claude's own transcript at
...
with fresh uuids; the returned id map is applied to Mack's retained turns.
...
routes resolved through CC Switch. Mack probes it in the background and caches
...
permission request on its stream; its rules live in `amp permissions`, so Mack
...
**Branch** — `amp threads export <id>` dumps the thread, Mack keeps the retained
...
(`MACK_AMP_BRANCH_CONTEXT_V1`). Forking a thread that was itself seeded this way
...
([driver/opencode.rs](../crates/waku-core/src/driver/opencode.rs)). Mack already started this
...
only under `/api`, and since this server is Mack's alone, filtering by
...
to. Alongside Codex's app-server, this is the only transport where Mack's
...
capability, since Mack does not proxy the agent's file or terminal access — an
...
the same opt-in: Mack calls `cursor/list_available_models` after `initialize`
...
then Mack applies `thought_level`/`effort`, `thinking`, `fast`, and `context`
from the session's traits (or from that alias suffix). If an older Cursor
agent advertises no model option, Mack retains the legacy `session/set_model`
...
Mack first selects Fx's `gateway` provider option and reads the refreshed model
option from that response. Mack then targets the exact `model` id with
...
| `user_message_chunk` | ignored — Mack's own prompt echoed back |
...
than assistant content, so Mack filters that prelude from the transcript.
...
whole basis for the user's decision. Outside Supervised, Mack answers for the
...
services *Mack offers the agent*, not permissions the agent needs. `fs` exists so
an editor can serve unsaved buffer contents in place of what is on disk, and
`terminal` lets the agent run commands through the client's own terminal. Mack
...
Advertising a capability Mack cannot service is the harmful choice, because the
...
The one case that would justify serving `fs/read_text_file` is Mack's own file
...
**Access modes** — Fx exposes native `ask` and `code` modes, so Mack maps
Supervised to `ask` and the auto modes to `code`. Every other ACP agent stays in
its ordinary execution mode, and `auto_approve` decides whether Mack answers
...
name**, and the two disagree: Mack sends `mode` by default, but Kimi's `mode` is
...
the base model. Fast is Mack's service-tier control (`fast` vs Standard).
...
advertise steering. Follow-ups remain in Mack's queue and start after the
...
Mack's `RuntimeMode` (Supervised / Auto-accept edits / Auto / Full access)
maps into each CLI's own vocabulary.

| Mack | Codex (`approvalPolicy` / `sandbox` / reviewer) | Claude `--permission-mode` | Cursor | Fx | OpenCode | Grok | Kimi Code |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
`--yolo` bypasses the one it has. Only the last of those is Mack's own
limitation rather than the CLI's.

## Resume cursors

`ProviderResumeCursor` ([model.rs](../crates/waku-protocol/src/model.rs)) is
persisted with the session and is what makes a Mack task outlive its process:
...
| Provider | T3 Code transport | Mack transport |
| --- | --- | --- |
...
streaming-input protocol, which Mack speaks directly. No Node sidecar and no npm
dependency.

Mack goes one further than the comparison: Amp and Pi, which T3 Code does not
support, are long-lived here too. Every provider holds a session.

What the long-lived session buys, and what Mack pays for not having it:

| Capability | T3 Code | Mack |
| --- | --- | --- |
...
`capabilities` record. Mack's equivalent surface is split between
...
Note the parts that are *not* a gap. Mack's Codex path is the same app-server
