...
`~/.local/mack.app` and installs the desktop entry into
`~/.local/share/applications`, so **Mack appears in your applications menu** —
you can also launch it from a terminal via `mack` command. Run the script again to
...
- **glibc 2.35 or newer** — Ubuntu 22.04, Debian 12, Fedora 36, and anything
  ...
  build from source.
- **A working Vulkan or OpenGL driver.** Mack renders through wgpu, which tries
...
Set `MACK_VERSION` to install a specific version rather than the latest.

## Installing manually

The script is a convenience, not a requirement. Download
`mack-<version>-<target>.tar.gz` from
[releases.mack.sh](https://releases.mack.sh) or the
[GitHub release](https://github.com/ifubaraboye/mack/releases), then unpack it
wherever you like:

```sh
mkdir -p ~/.local/mack.app
tar -xzf mack-<version>-<target>.tar.gz --strip-components=1 -C ~/.local/mack.app
ln -sf ~/.local/mack.app/bin/mack ~/.local/bin/mack   # optional
```
...
**Keep `bin/` and `share/mack/` intact.** Mack launches its daemon, updater,
and Computer Use helpers from `bin/`; the SDK library and supporting resources
ship in the same installation. A symlink is fine — Mack resolves it back to
the real path.
...
and name (Mack reports the Wayland `app_id` / X11 `WM_CLASS` `sh.mack`, which
matches the entry's filename). Install the packaged file and point it at the
install (the packaged copy uses bare `Exec=mack` and `Icon=sh.mack` names so it
can be relocated):

```sh
install -D ~/.local/mack.app/share/applications/sh.mack.desktop \
  -t ~/.local/share/applications
sed -i "s|^Exec=mack$|Exec=$HOME/.local/mack.app/bin/mack|" \
  ~/.local/share/applications/sh.mack.desktop
sed -i "s|^Icon=sh.mack$|Icon=$HOME/.local/mack.app/share/icons/hicolor/256x256/apps/sh.mack.png|" \
  ~/.local/share/applications/sh.mack.desktop
```

## Updating

Tarball installs under the user's home directory update themselves. Mack
checks once per launch by default; an available release appears in the sidebar
footer. Clicking it validates the staged installation, quits through Mack's
...
- `https://releases.mack.sh/appcast-linux-x86_64.xml`
- `https://releases.mack.sh/appcast-linux-aarch64.xml`
...
manual fallback for the default `~/.local/mack.app` install.
...
This removes `~/.local/mack.app`, the symlink, and the desktop entry. Projects
and settings stay in `~/.mack`; delete that directory to remove them too.
...
in the driver, not in Mack, and no application-side setting avoids it.
