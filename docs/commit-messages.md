...
When the commit dialog's message box is left empty, Mack generates the subject
line by running **the session's own agent CLI** once, headlessly. No coding
agent exposes a commit-message endpoint and Mack holds no provider API key, so
...
- **Amp** has no flag that disables tools, so Mack writes a throwaway settings
...
- **Claude Code** runs in `plan` permission mode with an empty `--tools` list,
  ...
  tiers in Mack's catalog, but the CLI still accepts `--effort low` for it.
