...
How each session in the sidebar gets its name: who writes the title, how Mack
...
UI, on its own cheap model, as part of work it is doing anyway. Mack's job is
...
2. **Polled from a native store.** The agent writes the title to disk (or to a
   ...
   Grok work this way.
3. **Generated by Mack.** Only Codex.
...
| Codex CLI | **Mack** (`gpt-5.6-luna`, effort `low`) | second `codex app-server` process | first `turn/started`, new threads only | [codex.rs:636](../crates/waku-core/src/driver/codex.rs#L636) |
...
`generate_session_title` to Sonnet while the turn stayed on Opus. Mack does not
...
Mack writes when it forks a session — and it wins by being later in the file.

Mack reads it on two paths:
...
### Codex — the one Mack generates

The Codex app-server **persists** a thread name but never generates one; naming
is the client's job, and Codex Desktop does it client-side too. So Mack matches
that behavior in
...
- **OpenCode** — `session.updated` → `properties./info/title`. Titles beginning
  ...
  Mack's prompt fallback with something strictly less useful.
...
the start of the first turn and Mack forwards the `title` as `AutoTitleUpdated`,
no polling and no extra process.

What it sends, though, is **the first prompt verbatim** — the same content as
Mack's own fallback, arriving through a different door. Kimi files it in its
