...
How Mack keeps CPU flat while a provider streams, what each piece of the
...
The root `Mack` view re-renders on every frame regardless of what is dirty, so
it must stay thin: the sidebar, transcript, and right panel are `MackPane`
