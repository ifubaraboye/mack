...
const RELEASES_BASE = 'https://releases.mack.sh'
...
export const FALLBACK_DOWNLOAD_URL = `${RELEASES_BASE}/Mack-0.0.1.dmg`
...
export function windowsInstallerUrl(version: string, arch: string) {
  return `${RELEASES_BASE}/Mack-${version}-${arch}-Setup.exe`
}
...
const fetchLatestRelease = createServerFn({ method: 'GET' }).handler(
  async (): Promise<LatestRelease | null> => {
    try {
      ...
      return {
        version,
        url: `${RELEASES_BASE}/Mack-${version}.dmg`,
        pubDate,
      }
    } catch {
      return null
