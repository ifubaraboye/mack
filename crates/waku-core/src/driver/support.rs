...
fn build_opencode_computer_use_config(
    ...
    mcp.insert(
        ...
        serde_json::json!({
            ...
            "enabled": true,
            "environment": {
                "MACK_COMPUTER_USE_SERVER": server_path.display().to_string(),
                "MACK_COMPUTER_USE_PROCESS_DIRECTORY": process_directory.display().to_string(),
            },
        }),
    );
...
pub(super) fn opencode_computer_use_environment(
    ...
    vec![
        ...
        (
            "MACK_COMPUTER_USE_SERVER".to_owned(),
            base.server_path.display().to_string(),
        ),
        (
            "MACK_COMPUTER_USE_PROCESS_DIRECTORY".to_owned(),
...
fn build_grok_computer_use_config(
    ...
    let rules = fs::read_to_string(&base.skill_path).with_context(|| {
        format!(
            "could not read Mack Computer Use skill {}",
...
fn build_grok_computer_use_toml(
    ...
    environment.insert(
        "MACK_COMPUTER_USE_SERVER".into(),
        toml::Value::String(base.server_path.display().to_string()),
    );
    environment.insert(
        "MACK_COMPUTER_USE_PROCESS_DIRECTORY".into(),
    ...
    mcp_servers.insert("mack_js_repl".into(), toml::Value::Table(server));
...
/// a GLOBAL store shared with the user's own terminal, so a Full Access Mack
...
mod tests {
    ...
    fn computer_use_config() -> computer_use_runtime::ComputerUseConfig {
        computer_use_runtime::ComputerUseConfig {
            server_path: PathBuf::from("/tmp/Mack Computer Use"),
            repl_path: PathBuf::from("/Applications/Mack.app/Contents/Resources/mack_js_repl"),
            skill_path: PathBuf::from(
                "/Applications/Mack.app/Contents/Resources/skills/mack-computer-use/SKILL.md",
            ),
            process_directory: PathBuf::from("/tmp/mack-computer-use/session"),
        }
    }
    ...
    fn opencode_computer_use_config_preserves_existing_inline_config() {
        let content = build_opencode_computer_use_config(
            ...
            Path::new("/Applications/Mack Computer Use"),
            Path::new("/Applications/Mack.app/Contents/Resources/mack_js_repl"),
            Path::new(
                "/Applications/Mack.app/Contents/Resources/skills/mack-computer-use/SKILL.md",
            ),
            Path::new("/tmp/mack computer use/session"),
        )
        .unwrap();
        ...
        assert_eq!(
            value
                .pointer("/mcp/mack_js_repl/command/0")
                .and_then(Value::as_str),
            Some("/Applications/Mack.app/Contents/Resources/mack_js_repl")
        );
        assert_eq!(
            value
                .pointer("/mcp/mack_js_repl/environment/MACK_COMPUTER_USE_SERVER")
                .and_then(Value::as_str),
            Some("/Applications/Mack Computer Use")
        );
        assert_eq!(
        ...
        assert!(value.pointer("/mcp/mack_computer_use").is_none());
    }

    #[test]
    fn grok_computer_use_config_preserves_existing_config_and_replaces_mack_server() {
        ...
        let server = value
            .get("mcp_servers")
            .and_then(|mcp| mcp.get("mack_js_repl"))
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            server
                .get("env")
                .and_then(|env| env.get("MACK_COMPUTER_USE_SERVER"))
                .and_then(toml::Value::as_str),
            Some("/tmp/Mack Computer Use")
        );
    }
