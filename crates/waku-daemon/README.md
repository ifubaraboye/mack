...
`waku-daemon` is the standalone process that hosts Mack's provider sessions.
It defaults to a loopback-only listener, authenticates clients with
`MACK_DAEMON_TOKEN`, and
prints one JSON readiness record to stdout containing its address, protocol
version, and process ID.

```text
MACK_DAEMON_TOKEN=<secret> mack-daemon --bind 127.0.0.1:0 [--parent-pid PID] [--allow-origin ORIGIN]...
```

Mack Desktop supervises this process. Debug builds use the feature-gated
`mack-debug-daemon` target at `target/debug/mack-debug-daemon`, so rebuilding
provider code replaces only the daemon. Release distributions place the signed
`mack-daemon` binary beside the desktop executable.

The token is a full-control capability for a trusted Mack client, not a user or
...
Mack Desktop adds that flag only after the user enables exposure in Settings →
