...
//! itself with a production build. `MACK_PREVIEW_UPDATE=1` fakes only the
//! automatic sidebar result while retaining the real Sparkle flow for the
//! Check for Updates menu; `MACK_FORCE_UPDATER=1` exercises everything for
...
/// The compact state rendered by Mack. Update details remain owned by
...
mod macos {
    ...
    struct UserDriverIvars {
        ...
        /// use Sparkle's own windows. Scheduled checks stay inside Mack.
    ...
    impl Updater {
        ...
        pub fn init() -> Option<Self> {
            let preview = cfg!(debug_assertions)
                && std::env::var_os("MACK_PREVIEW_UPDATE").is_some_and(|value| value == "1");
            let forced = std::env::var_os("MACK_FORCE_UPDATER").is_some_and(|value| value == "1");
            ...
            if handle.is_null() {
                ...
                eprintln!("Mack updater: failed to load Sparkle: {reason}");
                return None;
            }
            ...
            if !started {
                eprintln!("Mack updater: Sparkle rejected its updater configuration");
                return None;
            }
    ...
    /// (Contents/MacOS/Mack → Contents/Frameworks/Sparkle.framework/Sparkle).
    ...
    mod tests {
        ...
        fn routing_user_driver_satisfies_sparkle_protocols() {
            ...
            let library = target_dir
                .join("debug/Mack Debug.app/Contents/Frameworks/Sparkle.framework/Sparkle");
            if !library.exists() {
                return;
            }
...
mod feed {
    ...
    /// Compare dotted release numbers field by field. Mack's versions are
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <item>
      ...
      <enclosure url="https://releases.mack.sh/Mack-0.1.4-x86_64-Setup.exe" length="1024" type="application/octet-stream" sparkle:edSignature="oldsig" />
    </item>
    <item>
      <title>0.2.0</title>
      <sparkle:shortVersionString>0.2.0</sparkle:shortVersionString>
      <enclosure url="https://releases.mack.sh/Mack-0.2.0-x86_64-Setup.exe" length="2048" type="application/octet-stream" sparkle:edSignature="newsig" />
    </item>
  </channel>
</rss>"#;
        ...
        fn the_newest_signed_item_wins_regardless_of_feed_order() {
            ...
            assert!(item.url.ends_with("Mack-0.2.0-x86_64-Setup.exe"));
        }
...
mod windows {
    ...
    const FEED_URL: &str = "https://releases.mack.sh/appcast-windows-aarch64.xml";
    #[cfg(not(target_arch = "aarch64"))]
    const FEED_URL: &str = "https://releases.mack.sh/appcast-windows-x86_64.xml";

    /// Read out of `resources/Info.plist` by the build script, so macOS and
    /// Windows cannot end up trusting different keys.
    const PUBLIC_ED_KEY: &str = env!("MACK_SPARKLE_PUBLIC_ED_KEY");
    ...
    impl Updater {
        pub fn init() -> Option<Self> {
            ...
            if verifying_key().is_none() {
                eprintln!("Mack updater: SUPublicEDKey is not a valid ed25519 key");
                return None;
            }
        ...
        pub fn set_automatically_checks_for_updates(&self, enabled: bool) {
            ...
            let _ = std::thread::Builder::new()
                .name("mack-updater-preference".into())
    ...
    fn download_and_verify(item: &AppcastItem) -> anyhow::Result<PathBuf> {
        ...
        let installer = directory.join("Mack-Setup.exe");
    ...
    mod tests {
        ...
        fn a_signature_from_the_release_script_verifies_here() {
            ...
            const PAYLOAD: &[u8] = b"Mack-0.0.0-x86_64-Setup.exe contents";
        ...
        fn an_absent_preference_file_leaves_automatic_checks_on() {
            let directory = std::env::temp_dir()
                .join(format!("mack-updater-preference-{}", std::process::id()));
            let _ = std::fs::remove_dir_all(&directory);
