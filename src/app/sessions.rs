...
impl Waku {
    ...
    /// Delete a project and every chat it holds, from the palette's
    /// Projects view. Each chat goes through the same teardown as removing
    /// it on its own; the selection is parked outside first so no removal
    /// triggers its re-homing path mid-loop.
    pub(super) fn delete_project(&mut self, project_id: Uuid, cx: &mut Context<Self>) {
        ...
        let member_ids: Vec<Uuid> = self
            ...
            .map(|session| session.id)
            .collect();
        // A chat mid-fork cannot be torn down safely; refuse the whole
        // delete rather than strand it under a missing project.
        if member_ids
            .iter()
            .any(|session_id| self.response_fork_preparations.contains_key(session_id))
        {
            self.show_toast(tr!("session.response_fork_in_progress"));
            cx.notify();
            return;
        ...
        if self
            .state
            .selected_session
            .is_some_and(|selected| member_ids.contains(&selected))
        {
            match self
                .state
                .sessions
                ...
                .map(|session| session.id)
            {
                ...
                None => self.state.selected_session = None,
            }
        }
        ...
        cx.notify();
        if member_ids.len() == 1 {
            self.show_success_toast(tr!("project.deleted_one", name = project_name));
        } else {
            self.show_success_toast(
                tr!("project.deleted_many", name = project_name, count = member_ids.len()),
            );
        }
    }
