...
actions!(mack_goal_dialog, [ConfirmGoalDialog, DismissGoalDialog]);
...
impl Mack {
    ...
    fn materialize_goal_dialog(
        ...
        // Like Mack's other deferred surfaces, the modal joins the dispatch
    ...
    /// there before the first message. Mack starts providers lazily, so the
    ...
    fn begin_goal_pursuit_turn(
        ...
        cx.spawn(async move |mack, cx| {
            ...
            let _ = mack.update(cx, |mack, cx| {
                ...
                if stale {
                    mack.unwind_unconfirmed_pursuit_turn(session_id);
                    cx.notify();
                }
            });
        })
    ...
    pub(super) fn render_goal_dialog(
        ...
        let mut card = div()
            ...
            .on_action(cx.listener(|mack, _: &ConfirmGoalDialog, window, cx| {
                mack.confirm_goal_dialog(window, cx);
            }))
            .on_action(cx.listener(|mack, _: &DismissGoalDialog, window, cx| {
                mack.close_goal_dialog(window, cx);
            }))
            .tab_group()
            .tab_stop(false)
            .w_full()
        ...
        let save = render_goal_action_row(
            ...
            weak.clone(),
            &theme,
            |mack, window, cx| mack.confirm_goal_dialog(window, cx),
        );
        ...
        if let Some((status, label, icon_path)) = status_action {
            ...
            actions_column = actions_column.child(render_goal_action_row(
                ...
                toggle_weak,
                &theme,
                move |mack, window, cx| mack.goal_dialog_set_status(status, window, cx),
            ));
        }
        if current.is_some() && !replace {
            ...
            actions_column = actions_column.child(render_goal_action_row(
                ...
                clear_weak,
                &theme,
                |mack, window, cx| mack.goal_dialog_clear(window, cx),
            ));
        }
        let card = card
        ...
        let layer = div()
            ...
            .on_mouse_down(
                MouseButton::Left,
                cx.listener(|mack, _, window, cx| mack.close_goal_dialog(window, cx)),
            )
            .child(card);
...
fn render_goal_action_row(
    ...
    theme: &Theme,
    activate: impl Fn(&mut Mack, &mut Window, &mut Context<Mack>) + Clone + 'static,
) -> Stateful<Div> {
    ...
    div()
        ...
        .when(enabled, |row| {
            row.on_click(move |_, window, cx| {
                let _ = click_weak.update(cx, |mack, cx| click_activate(mack, window, cx));
            })
            .on_key_down(move |event: &KeyDownEvent, window, cx| {
                if !event.keystroke.modifiers.modified()
                    && matches!(event.keystroke.key.as_str(), "enter" | "space")
                {
                    let _ = key_weak.update(cx, |mack, cx| activate(mack, window, cx));
                    cx.stop_propagation();
                }
            })
        })
