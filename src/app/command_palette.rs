...
impl Mack {
    ...
    fn command_palette_commands(&self, searching: bool) -> Vec<CommandPaletteItem> {
        ...
        for (page, label_key, icon, keywords) in [
            ...
            (
                ...
                "settings preferences usage tokens",
            ),
            (
                SettingsPage::Daemon,
                "settings.daemon",
    ...
    fn open_command_palette_resume_view(
        ...
        cx.spawn(async move |mack, cx| {
            ...
            let _ = mack.update(cx, |mack, cx| {
                if !mack.command_palette.open
                    || mack.command_palette.provider_session_generation != generation
                    || mack.command_palette.resume_provider != provider
                {
                    return;
                }
                mack.command_palette.provider_sessions_pending = false;
                match result {
                    Ok(sessions) => {
                        mack.command_palette.provider_sessions = sessions;
                        mack.command_palette.provider_session_error = None;
                    }
                    Err(error) => {
                        mack.command_palette.provider_sessions.clear();
                        mack.command_palette.provider_session_error = Some(error.to_string());
                    }
                }
                if mack.command_palette.view == CommandPaletteView::Resume {
                    let query = mack.command_palette.search.read(cx).content().to_owned();
                    mack.refresh_command_palette_results(&query, false, cx);
                }
                cx.notify();
            });
        })
    ...
    fn load_command_palette_provider_session(
        ...
        cx.spawn(async move |mack, cx| {
            let result = cx.background_executor().spawn(async move { fetch() }).await;
            let update = mack.update(cx, |mack, cx| {
                if !mack.command_palette.open
                    || mack.command_palette.view != CommandPaletteView::Resume
                    || mack.command_palette.provider_session_generation != generation
                    || mack
                        .command_palette
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        mack.command_palette.provider_session_import = None;
                        mack.command_palette.provider_session_error = Some(error.clone());
                        mack.show_toast(tr!("command_palette.resume_failed", error = error));
                        cx.notify();
                        None
                    }
                }
            ...
            let _ = window_handle.update(cx, |_, window, cx| {
                let _ = mack.update(cx, |mack, cx| {
                    if mack.command_palette.open
                        ...
                        && mack.command_palette.provider_session_generation == generation
                    {
                        mack.import_provider_session(summary, history, window, cx);
                    }
                });
            });
        })
