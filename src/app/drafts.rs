    ...
    pub(super) fn schedule_composer_draft_save(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |mack, cx| {
            cx.background_executor()
                ...
                .await;
            let Some((store, drafts)) = mack
                .update(cx, |mack, cx| {
                    if mack.composer_draft_save_generation != generation {
                        return None;
                    }
                    mack.capture_current_composer_draft(cx);
                    Some((
                        mack.composer_draft_store.clone(),
                        mack.composer_drafts.clone(),
                    ))
                })
                .ok()
                .flatten()
            ...
            if let Err(error) = result {
                let _ = mack.update(cx, |mack, cx| {
                    mack.show_toast(tr!("errors.save_local_state", error = error));
                    cx.notify();
                });
            }
        })
