//! Signed, rollback-safe updates for Mack's managed Linux tarball install.
//!
//! Checks, downloads, signature verification, and extraction all run on a
//! worker thread. Once an archive is fully staged, `mack-updater` validates
...
// No release domain exists yet, so there is no feed to check: the updater
// stays dormant and users upgrade by downloading a newer AppImage or
// tarball. When the domain lands, point these at
// `https://releases.mack.sh/appcast-linux-<arch>.xml` (see RELEASING.md).
...
impl InstallLayout {
    fn discover() -> Option<Self> {
        ...
        if executable.file_name()? != "mack" || executable.parent()?.file_name()? != "bin" {
            return None;
        }
...
impl Updater {
    ...
    pub fn install_available_update(&self) -> bool {
        ...
        if std::thread::Builder::new()
            .name("mack-updater-handoff".into())
            .spawn(move || {
...
fn extract_release_archive(path: &Path, destination: &Path, version: &str) -> anyhow::Result<()> {
    ...
    let expected_root = format!("mack-{version}-{triple}");
...
fn validate_packaged_layout(prefix: &Path) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        ...
        "the install is not marked as a Mack-managed tarball"
    );
...
fn validate_download_url(value: &str) -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        url.scheme() == "https"
            && url.host_str() == Some("releases.mack.sh")
            && url.username().is_empty()
            && url.password().is_none(),
        "the update feed points outside releases.mack.sh"
    );
    Ok(())
}
...
mod tests {
    ...
    fn extraction_strips_only_the_expected_release_root() {
        ...
        let root = format!("mack-9.8.7-{triple}");
        ...
        for (relative, contents, mode) in [
            ("bin/mack", b"app".as_slice(), 0o755),
            ("bin/mack-daemon", b"daemon".as_slice(), 0o755),
            ("bin/mack-updater", b"helper".as_slice(), 0o755),
        ...
        assert_eq!(fs::read(output.join("bin/mack")).unwrap(), b"app");
