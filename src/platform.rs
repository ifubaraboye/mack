...
#[cfg(target_os = "linux")]
pub fn init_reduce_motion(cx: &mut gpui::App) {
    if let Ok(value) = std::env::var("MACK_REDUCE_MOTION")
...
/// delegate (and therefore click responses); Mack only supplies content here
...
/// Keep Mack's single main window alive when the user closes it. This preserves
